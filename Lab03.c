...
// push() for pushing an expression in the Stack.
...
// pop() for deleting/removing the expression from the Stack
...
int main()
{
    ...
    gets(exp);                     //when we have to input string the gets is used                  
    expSize = strlen(exp);         // this shows string size.   
    s.Size = expSize;       
